...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.CreateEVARawTransaction(wrapper, rawTx)
}

//SignRawTransaction 签名交易单
func (decoder *TransactionDecoder) SignRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.SignEVARawTransaction(wrapper, rawTx)
}

//VerifyRawTransaction 验证交易单，验证交易单并返回加入签名后的交易单
func (decoder *TransactionDecoder) VerifyRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.VerifyEVARawTransaction(wrapper, rawTx)
}
...
func (decoder *TransactionDecoder) CreateEVARawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	for _, a := range addressesBalanceList {
		if a.Balance.Cmp(amount) < 0 {
			...
			if count.Cmp(amount) >= 0 {
				...
				log.Error("The EVA of the account is enough,"+
	...
	message := []evaioTransaction.Message{evaioTransaction.NewMessage(messageType, evaioTransaction.NewMsgSend(from, to, evaioTransaction.Coins{evaioTransaction.NewCoin(denom, int64(convertFromAmount(amountStr)))}))}
	txStruct := evaioTransaction.NewTxStruct(chainID, memo, accountNumber, int(sequence), &txFee, message)
...
func (decoder *TransactionDecoder) SignEVARawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			//签名交易
			/////////交易单哈希签名
			sig, err := evaioTransaction.SignTransactionHash(keySignature.Message, keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyEVARawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	pass := evaioTransaction.VerifyTransactionSig(emptyTrans, signature, point)
